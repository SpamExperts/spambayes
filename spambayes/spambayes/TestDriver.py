...
def printhist(tag, ham, spam, nbuckets=options["TestDriver", "nbuckets"]):
    ...
    for tag, (h, s) in info:
        ...
        print('->     fp rate %.3g%%; fn rate %.3g%%; unsure rate %.3g%%' % (
              old_div(num_fp*1e2, ham.n), old_div(num_fn*1e2, spam.n),
              old_div((num_unh + num_uns)*1e2, (ham.n + spam.n))))
...
class Driver(object):
    ...
    def alldone(self):
        ...
        print("-> <stat> all runs false positive %:", (old_div(nfp * 1e2, nham)))
        print("-> <stat> all runs false negative %:", (old_div(nfn * 1e2, nspam)))
        print("-> <stat> all runs unsure %:", (old_div(nun * 1e2, (nham + nspam))))
